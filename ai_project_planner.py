...
if st.button("Generate MS Project XML (Enhanced)"):
    ...
    with st.spinner("Generating MS Project XML via Aspose.Tasks (enhanced)..."):
        try:
            created = save_aspose_project(project_model, str(out_path))
        except Exception as e:
            tb = traceback.format_exc()
            st.error(f"❌ Failed to generate MS Project XML: {e}")
            st.code(tb)
            st.stop()
